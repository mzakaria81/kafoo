...
abstract class AppLocalizations {
  ...
  /// The line the assistant says on arriving at the Cook's Meal list, and shown in the spoken banner as its receipt. AN ICU PLURAL RATHER THAN A FIXED NOUN, BECAUSE ARABIC COUNTS IN FIVE BANDS AND THE NOUN CHANGES WITH THE NUMBER: one=أكلة واحدة, two=أكلتين, few (3-10)=أكلات, many (11-99)=أكلة, other (100+)=أكلة. It shipped saying «عندك 3 أكلة» for every count, which no Egyptian writes and no Egyptian says; it survived because every test used a count in the 11-99 band, the one band where the singular is right. `published` is 0 whenever every Meal is still a draft, and «منهم 0» is not a sentence, so nothing-on-the-menu is said in words. `total` is never 0 — an empty list renders MyMealsEmpty instead. DIGITS ARE STILL DIGITS: the provider reads «20» as «عشرين» itself, so Kafoo needs no Arabic number-to-words function for the voice, only correct agreement around the number. FLAG: wants a native Cairene ear on two things — whether «3 أكلات» is read «تلات أكلات» rather than «تلاتة أكلات», and whether «عندك أكلة واحدة، منهم واحدة على المنيو» is how a Cairene would say it when a Cook has exactly one Meal.
  ///
  /// In ar, this message translates to:
  /// **'{addressForm, select, feminine{عندك {total, plural, one{أكلة واحدة} two{أكلتين} few{{total} أكلات} many{{total} أكلة} other{{total} أكلة}}، {published, plural, =0{بس مفيش ولا واحدة على المنيو} one{منهم واحدة على المنيو} two{منهم اتنين على المنيو} other{منهم {published} على المنيو}}. عايزة تعملي إيه؟} other{عندك {total, plural, one{أكلة واحدة} two{أكلتين} few{{total} أكلات} many{{total} أكلة} other{{total} أكلة}}، {published, plural, =0{بس مفيش ولا واحدة على المنيو} one{منهم واحدة على المنيو} two{منهم اتنين على المنيو} other{منهم {published} على المنيو}}. عايز تعمل إيه؟}}'**
