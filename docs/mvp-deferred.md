...
| 22 | 2026-08-14 | **The live voice agent's instructions live in the ElevenLabs dashboard, not in this repository** | ElevenLabs agent config, `prompts/conversation.md`, `prompts/kitchen-conversation.md` | Two prompts govern the same journey and only one is in version control. The typed path runs Kafoo's prompt through its own function; the orb runs whatever is configured on the provider's account, which nobody here can read, review or roll back. A change to the register, the refusal to guess a form of address, or the injection rules reaches only half the Cooks | The custom-model endpoint of row 15 lands, or the founder signs the ElevenLabs connector in so the agent can be read from a session |
