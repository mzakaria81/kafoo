...
| `README.md` | The designer's handoff notes — what each reference file shows, the state machine, the accessibility floor, and the list of known gaps. |
| `references/*.dc.html` | Design references in HTML. Prototypes of intended look and behaviour. **Not production code — do not port them.** Open them in a browser; the tap-target, sunlight and state-cycling toggles are worth using. |
| `screenshots/` | Static previews, for orientation only. `DESIGN.md` and the HTML are authoritative. |
...
`apps/mobile/assets/fonts/` carries IBM Plex Sans Arabic in four weights under
the SIL Open Font License, declared in that app's `pubspec.yaml`, and
`KafooType.fontFamily` names it. `apps/web/` still falls
