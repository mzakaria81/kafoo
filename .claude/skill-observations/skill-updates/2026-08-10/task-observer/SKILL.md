...
### Name the destination before writing the fix

Documentation gravitates to whichever file was open when the lesson was
learned, not to the file where the rule belongs. Where a project has both an
always-loaded instruction file (`CLAUDE.md`, `AGENTS.md` and equivalents) and
on-demand skills or path-scoped rules, the always-loaded file wins that
default — it is what the agent was reading when the correction arrived.

The cost is invisible per edit and compounds. Every session then pays for
every past incident, including the sessions that will never touch the
subject; and because each individual addition was correct, nothing signals
the growth. One audited instruction file had reached 631 lines with 36% of it
a procedure that was suspended at the time and already covered by an
installed skill.

So state the destination and why, before writing anywhere:

- **Always-loaded file** — only if it is needed in *every* session: a
  non-negotiable, a vocabulary rule, a trap with no other guard.
- **The procedure's own skill or reference file** — a multi-step workflow, an
  account or tooling detail, anything scoped to one kind of task. This is the
  default for a correction to a procedure, **even when that procedure's file
  is not the one you currently have open**.
- **A path-scoped rule** — guidance that only applies while working in one
  part of the tree.
- **A hook, test or check** — anything verifiable mechanically. Prefer this
  to prose wherever the rule is binary; a rule nobody verifies decays into
  decoration, and a rule stated in two places drifts in one of them.

This binds the `Suggested improvement` field too. An improvement that does
not say where it lands invites the next session to guess, and it will guess
"the file I already have open".
...
| Where does the fix go? | Name the destination and justify it first — always-loaded only if every session needs it; a procedure's correction belongs in the procedure's own file even when that file is not open |
