...
run "format"        dart format --set-exit-if-changed --output=none .
run "analyze"       melos run analyze
run "tests"         melos run test
run "codegen drift" bash -c 'dart run build_runner build --delete-conflicting-outputs >/dev/null 2>&1 && git diff --quiet -- "*.g.dart" "*.freezed.dart"'
